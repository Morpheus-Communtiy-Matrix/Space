<br>

<div align = center>

[![Join Link]][matrix-to]   
[![Join Matrix]][matrix]   
[![Matrixrooms Info]][matrixrooms]   

<br>

# Morpheus Community Matrix Space
</div>

Im [Morpheus Community Matrix Space](https://matrixrooms.info/room/morpheus_community_matrix:matrix.org) ([#morpheus_community_matrix:matrix.org](https://matrix.to/#/#morpheus_community_matrix:matrix.org)) tauscht sich die Community von [TheMorpheus](https://the-morpheus.de) aus. Diskutiere mit anderen über alle möglichen Themen der IT, bleib in [#Announcements](https://matrix.to/#/#morpheus-announcements:mikaff.de) auf dem Laufendem, was bei Morpheus und im Space passiert und Fachsimpel über Gott und die Welt in [#offtopic](https://matrix.to/#/!pYf323bvNx_DeAmJzoEGYAMhxsJFjeMlctUwKyL3OiQ?via=mikaff.de&via=matrix.org&via=tchncs.de). Aber sei dir gewiss Morpheus sieht alles, was in der Matrix geschieht.

Ließ die [Regeln](RULES.md) des Spaces und schau dir die [Struktur](STRUCTURE.md) unserer Räume und Subspaces an.

Wir verwenden folgende Bots in unserem Space:
* [Draupnir](Bots/Draupnir/Draupnir.md)
* [Morpheus Link Bot](Bots/Link%20Moderation/Link%20Moderation.md)
* [Podcast](Bots/Podcast/Podcast.md)
* [Youtube](Bots/Youtube/Youtube.md)

<br>
<br>
<br>

Alle Inhalte dieses Repos stehen unter CC-BY-NC 4.0 (Creative Commons Attribution-NonCommercial 4.0) siehe [LICENSE](LICENSE) und [creativecommons.org/licenses/by-nc/4.0](https://creativecommons.org/licenses/by-nc/4.0/).

[matrix-to]: https://matrix.to/#/!dEhdih-PF7uPLP5sw8NDYlqF8fSPvuW6BhGO6QqIggI?via=ipanema.chat&via=matrix.vulkanlan.at&via=matrix.terjokan.de&via=matrix.reddave.de&via=datoxi.de&via=frei.chat&via=nyaaa.de&via=palaven.net&via=matrix.tu-berlin.de&via=matrix.uni-hannover.de
[matrix]: matrix:roomid/dEhdih-PF7uPLP5sw8NDYlqF8fSPvuW6BhGO6QqIggI?action=join&via=matrix.buecherwiesel.at&via=matrix.bur-tech.de&via=skysteel.highwindtech.de&via=matrix.cs-homelab.de&via=matrix.the-morpheus.xyz&via=nitr.dev&via=matrix.luxasiix.org&via=matrix.behle-tech.de&via=meericedes.de&via=nebendahl.eu
[matrixrooms]: https://matrixrooms.info/room/morpheus_community_matrix:matrix.org

[Join Link]: https://img.shields.io/badge/Join-Matrix?style=flat-square&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%2CPD94bWwgdmVyc2lvbj0iMS4wIiBlbmNvZGluZz0iVVRGLTgiIHN0YW5kYWxvbmU9Im5vIj8%%2BCjwvc3ZnPgo%3D&logoSize=auto&labelColor=white&color=white

[Join Matrix]: https://img.shields.io/badge/Join-Matrix?style=flat-square&logo=matrix&logoColor=%23ffffff&logoSize=auto&labelColor=%23e08500&color=%23e08500

[Matrixrooms Info]: https://img.shields.io/badge/-info?style=flat-square&label=matrixrooms.info&labelColor=%231d293d&color=%2300000000
